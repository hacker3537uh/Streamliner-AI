...
def load_config() -> AppConfig:
    ...
    if not config_path.exists():
        raise FileNotFoundError("El archivo 'config.yaml' no se encuentra. Copia 'config.yaml.example' y ajústalo.")
    ...
    scoring_yaml = detection_yaml.pop("scoring", {}) # Extraemos y eliminamos 'scoring' de 'detection_yaml'
    ...
    config = AppConfig(
        ...
        storage=StorageConfig(
            ...
            aws_s3_endpoint_url=os.getenv("AWS_S3_ENDPOINT_URL") if storage_type == 'r2' else None,
        ),
        ...
        detection=DetectionConfig(
            **detection_yaml, # Pasamos los argumentos de 'detection' (ya sin 'scoring')
            scoring=ScoringConfig(**scoring_yaml) # Pasamos el 'scoring' construido por separado
        ),
