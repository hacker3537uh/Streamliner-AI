...
async def _extract_audio(video_path: Path) -> Path:
    ...
    logger.info(
        f"Extrayendo y convirtiendo audio de '{video_path}' a '{audio_path}' (formato WAV)..."
    )
    ...
    args = [
        ...
        "-vn",  # No video
        "-acodec",
        "pcm_s16le",  # Formato estándar para WAV (16-bit)
        "-ar",
        "16000",  # Frecuencia de muestreo de 16kHz (suficiente para voz)
        "-ac",
        "1",  # Convertir a mono (un solo canal)
        str(audio_path),
    ]
...
async def process_single_file(
    ...
    try:
        ...
        if not highlights:
            logger.warning(
                f"No se encontraron highlights en '{video_path.name}'. Finalizando proceso."
            )
            return
        ...
        for i, highlight in enumerate(clips_to_process):
            ...
            try:
                ...
                cutter = VideoCutter()
                clip_filename = (
                    video_path.parent / f"{video_path.stem}_clip_{i + 1}.mp4"
                )
                ...
                with open(srt_path, "w", encoding="utf-8") as srt_file:
                    for segment in transcription["segments"]:
                        ...
                        srt_file.write(
                            f"{int(start // 3600):02}:{int(start % 3600 // 60):02}:{int(start % 60):02},{int(start % 1 * 1000):03} --> "
                        )
                        srt_file.write(
                ...
                final_clip_path = (
                    video_path.parent / f"{video_path.stem}_final_{i + 1}.mp4"
                )
                # temp_files.append(final_clip_path)

                await renderer.render_vertical_clip(
                    str(cut_clip_path), str(final_clip_path), str(srt_path)
                )
                ...
                if config.storage.storage_type in ["s3", "r2"]:
                    remote_key = await storage.upload(
                        final_clip_path, final_clip_path.name
                    )
                    ...
                    await publisher.upload_clip(
                        str(final_clip_path), streamer, dry_run=dry_run
                    )
                ...
                for f in temp_files:
                    try:
                        ...
                        logger.warning(
                            f"No se pudo eliminar el archivo temporal {f}: {e}"
                        )
        ...
        if audio_path and os.path.exists(audio_path):
            try:
                ...
                logger.warning(
                    f"No se pudo eliminar el archivo de audio principal {audio_path}: {e}"
                )
