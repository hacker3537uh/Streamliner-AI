...
@cli.command()
@click.option('--file', 'video_path', required=True, type=click.Path(exists=True), help="Ruta al archivo de video local.")
@click.option('--streamer', required=True, help="Nombre del streamer para asociar el clip.")
@click.option('--dry-run', is_flag=True, help="Ejecuta todo el pipeline sin subir el video final.")
@click.pass_obj
...
def setup():
    ...
    click.echo("✅ Archivos de ejemplo `.env.template` y `config.yaml.example` están disponibles en el repositorio.")
