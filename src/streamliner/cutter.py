...
class VideoCutter:
    ...
    async def cut_clip(
        ...
        """
        logger.info(f"Cortando clip desde {start_seconds:.2f}s hasta {end_seconds:.2f}s...")
        ...
        args = [
            ...
            "-to", str(end_seconds),
            "-c", "copy",
            "-map", "0", # Copia todos los streams (video, audio, etc.)
            str(output_path)
        ]
        ...
        if process.returncode != 0:
            ...
            logger.error(f"Error al cortar el video con ffmpeg (código: {process.returncode}):")
